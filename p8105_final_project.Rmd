...
ggplot(cod_data, aes(x = year, y = percent_excess_death, fill = locality)) + geom_smooth() + geom_point(stat = "identity")
```

The regression model is percent_excess_death = 33.89 - 2.06*Metropolitan + 8.047Nonmetropolitsn. For people in metropolitan, the expected percent excess death has a decrease of 2.06. For people in non-metropolitan aream the expected percent excess death has an increase of 8.047. The p-values show these predictors are both significant. The small p-value might due to the large sample size. 
...
df <- cbind(map, rate = unique(map_cod_data$mean_ped))
...
ggplot(df, aes(map_id = id)) + 
  ...
  geom_text(aes(x = clong, y = clat, label = id))
```
